    ...
    try:
        ...
        for switch in deviceToNewPortUtilizationDataMap:
            if switch.find("l") > -1: # this means leaf switch
                ...
                for i in range (0,ConfigConst.MAX_TOR_SUBNET):
                    for j in range(int(ConfigConst.MAX_PORT_COUNT/2), ConfigConst.MAX_PORT_COUNT): # Only later half ports of a switch is connected to spines
                        port = i * ConfigConst.MAX_PORT_COUNT + j
                        util = utilData[port]
                        if(portToLoadMap.get(j+1) ==None):
                            portToLoadMap[j+1] = util
                        else:
                ...
                switchToLoadSTD[switch] = portLoadAsList

                #---- unnecessary part starts
                # print(imbalanceSpan )
        ...
        for switch in switchToLoadSTD.keys():
            print("Switch : "+str(switch)+ " Load info "+str(switchToLoadSTD.get(switch)))
...
getLoadImbalanceByFolder(
    "/home/deba/Desktop/P4TE/testAndMeasurement/LoadImbalanceAnalyzer/Hula_web_Search_STATISTICS_load_factor_0.8.log", "HULA", 0.8)