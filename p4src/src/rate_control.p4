                        ...
                        inout standard_metadata_t standard_metadata)
{
    ...
    //hash(local_metadata.flowlet_map_index, HashAlgorithm.crc16, (bit<13>)0, { hdr.ipv6.src_addr, hdr.ipv6.dst_addr, hdr.ipv6.flow_label}, (bit<13>)8191);
        ...
        if(hdr.tcp.syn_control_flag == FLAG_1){ //first pkt of the flow
            ...
            flowlet_id_to_seq_number_of_last_rate_control_action_map.read(local_metadata.last_seq_no_with_rate_control, (bit<32>)local_metadata.flowlet_map_index);
            if(local_metadata.last_seq_no_with_rate_control + SEQ_NUMBER_THRESHOLD_FOR_RATE_CONTROL < hdr.tcp.seq_no){
                // Only in this case we need to do rate control
                local_metadata.rate_control_allowed_for_the_tcp_flow = RATE_CONTROL_ALLOWED_FOR_THE_FLOW; //else rate control is not allowed and the vlaue is written in init_pkt of leaf.p4
        ...
        if (hdr.tcp.isValid() && (local_metadata.rate_control_allowed_for_the_tcp_flow == RATE_CONTROL_ALLOWED_FOR_THE_FLOW) &&
                (local_metadata.rate_control_event != RATE_CONTROL_EVENT_ALREADY_APPLIED )){
            ...
            flowlet_id_to_seq_number_of_last_rate_control_action_map.write((bit<32>)local_metadata.flowlet_map_index, hdr.tcp.seq_no); //TODO : we have to make sure that we are calling the flowlet computtion mandatoryly
             if ((local_metadata.egress_rate_event_hdr.egress_traffic_color >= (bit<32>) RED )  && (local_metadata.ingress_rate_event_hdr.ingress_traffic_color> (bit<32>) GREEN) ) {
                 local_metadata.rate_control_event = RATE_DECREASE_EVENT_NEED_TO_BE_APPLIED_IN_THIS_SWITCH ;
               ...
               local_metadata.rate_control_event =RATE_INCREASE_EVENT_NEED_TO_BE_APPLIED_IN_THIS_SWITCH ;

            }

            //log_msg("Updated a flow into rate control map");
        }else if ( hdr.tcp.isValid() && (hdr.tcp.ack_control_flag == FLAG_1 )  && (local_metadata.rate_control_event  == RATE_CONTROL_EVENT_ALREADY_APPLIED)){
                        ...
                        inout standard_metadata_t standard_metadata)
{
    ...
    //========This part is for actual flow control=================
        ...
        // Otherwise if this packet is a fake ack packet and we create another fake ack for this packet then this will create a repple effect. which we do not want
        // TODO : implement these logic using a table
        if (hdr.tcp.isValid() &&  (local_metadata.rate_control_allowed_for_the_tcp_flow == RATE_CONTROL_ALLOWED_FOR_THE_FLOW) && (local_metadata.rate_control_event != RATE_CONTROL_EVENT_ALREADY_APPLIED )){
                //This condition means, we are going to apply rate control in this switch. On what condition we decide to do rate control. Implement the logic here. Our taret is to add this logic through a table for easier expansion
...
//need to check if   local_metadata.rate_control_event ==RATE_CONTROL_EVENT_NEED_TO_BE_APPLIED_IN_THIS_SWITCH then we will clone the packet to the ingress port
  //  IS_CONTROL_PKT_TO_NEIGHBOUR macro te option add korte hobe
//and in the cloned version of the packet we will set  local_metadata.rate_control_event ==RATE_CONTROL_EVENT_ALREADY_APPLIED and forward the packet.
