...
control EgressPipeImpl (inout parsed_headers_t hdr,
    ...
    action build_p2p_feedback_with_fake_ack(){
        ...
        local_metadata.rate_control_event  = RATE_CONTROL_EVENT_ALREADY_APPLIED ;
       // hdr.ipv6.ecn = 3;
    }
    ...
    apply {
        ...
        if(!IS_NORMAL(standard_metadata)){
            ...
            if (standard_metadata.egress_port == PORT_ZERO) {
                ...
                #ifdef DP_BASED_RATE_CONTROL_ENABLED
                if (local_metadata.rate_control_event  == RATE_DECREASE_EVENT_NEED_TO_BE_APPLIED_IN_THIS_SWITCH){
                    build_p2p_feedback_with_fake_ack();
                }
                else if (local_metadata.rate_control_event  == RATE_INCREASE_EVENT_NEED_TO_BE_APPLIED_IN_THIS_SWITCH){
