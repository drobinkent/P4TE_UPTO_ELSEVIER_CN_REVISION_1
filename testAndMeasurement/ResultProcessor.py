...
def getMinMaxtimeStampForLInkVisulationforAFolder(folderPath):
    ...
    startTime,end, _  = parseIperfResultsFromFolder(folderPath)
...
def parseIperfResultsFromFolder(folderPath):
    ...
    return stime, endTime, val


def getOnlyUpwardPortUtilizationData(folderPath, deviceName, startTime=0, endTime=9999999999):
    '''
    ...
    for stats in controllerStatistics:
        if((stats.time >=startTime) and (stats.time <= endTime)):
            for p in stats.port_stats.upward_port_egress_packet_counter.keys():   # this is a port vs counter map
                ...
                if ( upwardPortToEgressCounterMap.get(p) == None):
                    timeCountTuple = (stats.time, upward_port_egress_packet_counter)
                    portVsTupleList = []
                    portVsTupleList.append(timeCountTuple)
                    upwardPortToEgressCounterMap[p] = portVsTupleList
                else:
                    if upwardPortToEgressCounterMap.get(p):
                        timeCountTuple = (stats.time, upward_port_egress_packet_counter)
                        portVsTupleList = upwardPortToEgressCounterMap.get(p)
                        portVsTupleList.append(timeCountTuple)
    return upwardPortToEgressCounterMap
...
def  upwardLinkUtilizationAnalyzer( config, folderPath ):
    min , max = getMinMaxtimeStampForLInkVisulationforAFolder(folderPath)
    # print("Link Util processing Start Time is : ", min)
    # print("Link Util processing End Time is : ", max)
    devToUpwardPortLinkUtilizationDataMap = {}
    for devName in config.nameToSwitchMap:
        val =getOnlyUpwardPortUtilizationData(folderPath = CC.CONTROLLER_STATISTICS_RESULT_FILE_PATH, deviceName=devName, startTime=min, endTime=max)
        devToUpwardPortLinkUtilizationDataMap[devName] = val
        # print("Device is ",devName)
        # print(val)
    return devToUpwardPortLinkUtilizationDataMap
...
def getMinMaxtimeStampOfTheIperfResults(iPerfResultObjectsForOneFolder):
    ...
    maxTimeStamp = maxTimeStamp + maxDuration
...
def processDataAndPLotUpwardLinkUtilizationGraph(valueSet1, valueSet2,  folderPathToStorePdf):
    for devName in valueSet1.keys():
        #for each device plot their upward linktulization graph
        portCountersForSet1 = valueSet1.get(devName)
        portCountersForSet2 = valueSet2.get(devName)
        #upwardPortToEgressCounterMap1 =
        print("Printing LinkUtilization for device "+str(devName))
        print("Printing Ecmp Port usage ")
        ecmpPortList = []
        ecmpValueList = []
        p4TEPortList = []
        p4TEValueList = []
        portListAsString = []
        for port in portCountersForSet1.keys(): # Same ports will be in both set
            portTxt = "port-"+str(port)
            portListAsString.append(str(port))
            ecmpPortList.append(int(port))
            dataSet1 = list(portCountersForSet1.get(port))
            ecmpValue = dataSet1[len(dataSet1)-1][1] - dataSet1[0][1]
            ecmpValueList.append(ecmpValue)
            print(portTxt+ " "+ str(ecmpValue))
        print("Printing P4TE Port usage")
        for port in portCountersForSet2.keys(): # Same ports will be in both set
            portTxt = "port-"+str(port)
            p4TEPortList.append(int(port))
            dataSet2 = list(portCountersForSet2.get(port))
            p4TEValue = dataSet2[len(dataSet2)-1][1] - dataSet2[0][1]
            print(portTxt+ " "+ str(p4TEValue))
            p4TEValueList.append(p4TEValue)

        if (len(ecmpValueList) <=0) or (len(p4TEValueList)<=0):
            print("No value to plot in graph")
        else:
            N = len(ecmpPortList)
            ind = np.arange(N)  # the x locations for the groups
            width = 0.35       # the width of the bars
            x1 = [i for i in ecmpPortList]
            y1 = [i for i in ecmpValueList]
            x2 = [i for i in p4TEPortList]
            y2 = [i for i in p4TEValueList]
            fig = plt.figure()
            ax = fig.add_subplot(111)
            # rects1 = ax.bar(x1, y1, width, color='r')
            rects1 = ax.bar(ind, y1, width, color='r',label="ECMP")
            # rects2 = ax.bar(x2, y2, width, color='b')
            rects2 = ax.bar(ind+width, y2, width, color='b',label="P4TE")
            ax.set_ylabel('No. of Packets Forwarded ')
            ax.set_xlabel("Port")
            # ax.set_title('Port vs Packets for '+devName)
            ax.set_xticks(ind+(width/2))
            ax.set_xticklabels( portListAsString )
